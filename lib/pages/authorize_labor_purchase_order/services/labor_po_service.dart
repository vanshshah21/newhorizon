...
class LaborPOService {
  ...
  Future<bool> authorizeLaborPOBatch(List<LaborPOData> poList) async {
    debugPrint("Authorizing Batch Labor PO");
    debugPrint("Count: ${poList.length}");
    final url = await StorageUtils.readValue('url');
    ...
    final response = await _dio.post('http://$url$endpoint', data: body);
    if (response.statusCode == 200 && response.data['success'] == true) {
      debugPrint("Batch authorization successful");
      return true;
    } else {
      debugPrint("Batch authorization failed");
      debugPrint("Status Code: ${response.statusCode}");
      debugPrint("Error: ${response.data}");
      return false;
    }
